...
void setup()
{
 ...
 dataFile = SD.open("Final.csv",FILE_WRITE);
     if(dataFile) {
    dataFile.println("UTC,Latitude,Londitude,N/S,E/W,Satellites,Status,A_x,A_y,A_z,Temp_IMU,G_x,G_y,G_z,th_x,th_y,th_z,Temp_BMP,Pressure");
   dataFile.close();  
   }
}
...
/*   if(!(MSL_Altitude[0]==0 && MSL_Altitude[1] < 1))
   {
   i = i +1;
   if(i==20){
     digitalWrite(2,HIGH);
